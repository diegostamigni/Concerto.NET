...
[StateMachineTransitionExecutorFor(ImportProfileTrigger.CreateCustomer)]
public class ImportProfileCreateCustomerStateMachineTransitionExecutor
	: IStateMachineTransitionExecutor<ImportProfileState, ImportProfileTrigger>
{
	private readonly IBus bus;
	private readonly IMapper mapper;

	public ImportProfileCreateCustomerStateMachineTransitionExecutor(IBus bus, IMapper mapper)
	{
		this.bus = bus;
		this.mapper = mapper;
	}
		...
		StateMachine<ImportProfileState, ImportProfileTrigger>.Transition transition)
	{
		...
		// Fire event
		// await this.bus.PubSub.PublishAsync<UploadAvatarEvent>(response);

		await stateMachineHandler.FireAsync(ImportProfileTrigger.Notify, response);
	}
}